...
simulation_condition = "2states_multi"
...
phase_shift_degree = [0, 0, 0, 180, 0, 0]
delays = [0, 0, 0, 0, 0, 100] # delay in ms                           
...
for sample_id in range(n_samples):
    for cond1_id, freq in enumerate(freq_ranges):
        for cond2_id, fs in enumerate(fs_range):
            for signal_pos_id, signal in enumerate(signal_positions):
                ...
                if gtype == 1: #empty channel
                    ...
                    signal_dict = simulate_independent_signal(
                        ...
                        burst_amp_sigma=burst_amp_sigma, rng=rng, prev_states=states_samples[sample_id, cond1_id, cond2_id, :signal_pos_id]
                        )
                elif gtype == 4:  #copy complete
                    signal_dict = {"signal": np.copy(signal_samples[sample_id, cond1_id, cond2_id, 0]),
                                   "states": np.copy(states_samples[sample_id, cond1_id, cond2_id, 0]),
                ...
                elif gtype == 5: #copy offset times with changed freq
                    signal_dict = {"signal": np.copy(signal_samples[sample_id, cond1_id, cond2_id, 0]),
                                   "states": np.copy(states_samples[sample_id, cond1_id, cond2_id, 0]),
                                   "bursts": np.copy(bursts_samples[sample_id, cond1_id, cond2_id, 0]),
                                   "unscaled_bursts": np.copy(unscaled_bursts_samples[sample_id, cond1_id, cond2_id, 0]),
                ...
                states_samples[sample_id, cond1_id, cond2_id, signal_pos_id]  = signal_dict["states"]
                bursts_samples[sample_id, cond1_id, cond2_id, signal_pos_id]  = signal_dict["bursts"]
                noise_samples [sample_id, cond1_id, cond2_id, signal_pos_id]  = signal_dict["noise"]
                signal_samples[sample_id, cond1_id, cond2_id, signal_pos_id]  = signal_dict["signal"]
                unscaled_bursts_samples[sample_id, cond1_id, cond2_id, signal_pos_id] = signal_dict["unscaled_bursts"]



print("Data shape", signal_samples.shape, "[samples, cond1, cond2, signals]: the data points are stored as object")

### calculate influence of signals to each other
